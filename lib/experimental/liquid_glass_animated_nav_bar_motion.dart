...
///
/// Everything else is the shipped choreography, untouched: the travel
/// spring, the morph-grow envelope, the appear ramp, the glass→static
/// cross-fade, the commit-on-arrival, and the capture lifecycle.
///
...
class LiquidGlassAnimatedNavBarMotion extends StatefulWidget {
  ...
  /// pill is hidden. Hosts that put lenses in [outerChild] must pass
  /// `true`.
...
class _LiquidGlassAnimatedNavBarMotionState
  ...
  /// The pill's live size, published by the pill itself. `null` means it
  /// is resting: no glass on screen, nothing for the capture to feed.
  ...
  void _onTick(Duration elapsed) {
    ...
    // 3) Arrived. Commit the selection and lower the pill — `active`
    // goes false, and the pill springs back down to its rest size on its
    // own. There is no cross-fade to run: the rest pill is the same body
    // with its cover faded up, so nothing is swapped for anything.
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      // Lifted whenever a finger is on the bar or the pill is travelling
      // to a tapped tab. Everything else — the morph spring, the
      // acceleration squash/stretch, and lowering back to the rest pill —
      // belongs to the pill.
      final bool lifted = _tabDragging || _travelActive;

      // What the pill reports it is drawing. Null means resting: no
      // glass, so the icon shell has nothing to reveal through and the
      // outer capture has nothing to feed.
      final Size? liveSize = _pillGeometry.value;

      final bool glassOn = liveSize != null && widget.showSelectionPill;
                  ...
                  Positioned.fill(
                    child: IgnorePointer(
                      child: LiquidGlassNavPill(
                        ...
                        // The rest pill is not a second widget any more:
                        // it is this pill's cover, opaque at rest and
                        // fading off as the glass rises under it. Nothing
                        // is ever swapped, so nothing can blink.
                        cover: LiquidGlassBottomNavPillStatic(
